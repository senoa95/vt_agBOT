...
#define BUTTON_PS3__L_TOP_FRONT    10     // top front left hand button
#define BUTTON_PS3__R_TOP_FRONT    11     // top front right hand button
#define BUTTON_PS3__SELECT         0     // Select button in center section on top
#define BUTTON_PS3__START          3     // Start button in center section on top

#define BUTTON_PS3__GRN_TRIANGLE   12     // Green triangle key to right
#define BUTTON_PS3__RED_CIRCLE     13     // Yellow 'Y' button 
#define BUTTON_PS3__BLUE_X         14     // Blue X button
#define BUTTON_PS3__VIO_SQUARE     15     // Violet square button

#define BUTTON_PS3__RESET          16   // Not implemented
...
// Joystick controls from /joy array for PS3
    ...
    int joystickConfig(int type) {
        ...
        switch (type) {
            case JOYSTICK_PS3:
                ...
                buttonMap.buttonCmdVel        = BUTTON_PS3__L_TOP_FRONT;  // Hold down to allow left joystick to /cmd_vel
                buttonMap.buttonHwDirect      = BUTTON_PS3__START;        // Hold down to use direct serial motor control
                buttonMap.buttonNavGotoTarget = BUTTON_PS3__SELECT;         // Hold down to 'shift' to nav goto target

                buttonMap.buttonForwardNav1   = BUTTON_PS3__GRN_TRIANGLE;
                buttonMap.buttonRightNav2     = BUTTON_PS3__RED_CIRCLE;
                buttonMap.buttonLeftNav3      = BUTTON_PS3__VIO_SQUARE;
                buttonMap.buttonStopNav4      = BUTTON_PS3__BLUE_X;

                buttonMap.buttonReset         = BUTTON_PS3__RESET;

                axisMap.axisLinear            = AXIS_PS3__LEFT_FWD_BACK;
                axisMap.axisAngular           = AXIS_PS3__LEFT_SIDE_SIDE;;
                break;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    int joyType = JOYSTICK_PS3;
    if (joy_input.setJoystickType(joyType) < 0) {
        ROS_ERROR("%s: Illegal joystick type of %d. PS3=%d",THIS_NODE_NAME,
            joyType, JOYSTICK_PS3);
    } else {
        ROS_INFO("%s: Joystick type is set to %d. PS3=%d",THIS_NODE_NAME,
              joyType, JOYSTICK_PS3);
    }
